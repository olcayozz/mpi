 ...
 //Her Kolon'u bir proses'e dagittik. 0'inci kolonu bu proseste cozecegiz.
  ...
  { /* WORKERS */
    ...
    for(int m=0;m<4; m++) //m fazları temsil ediyor. Total'de 4 kez shift yapilacak.
    {
	  ...
	  //int aShifter=(world_rank+m)%world_size+(i*world_size);
	  int aShifter=((i*world_size)+(m+i)%4)%16;

          local_cols_C[i]+=A[aShifter]*local_cols[i];//A'nin shift'i burada.
//if(world_rank==2 && i==1) printf("\n[%d] : aShifer = %d = A[%d] = i;%d, m;%d, world_rank;%d, world_size;%d\n",world_rank, A[aShifter],aShifter,i,m,world_rank,world_size);
...
/*
   if(world_rank>0)
	for(int i = 1 ; i<4 ; i++) 
 //Hesaplanan Her Kolon'u bir root'a gonder..
    {
    ...
    {  
        MPI_Recv(&cols2, 4, MPI_INT, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(&cols3, 4, MPI_INT, 0, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(&cols4, 4, MPI_INT, 0, 3, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	  for(int i=0;i<world_size;i++){        
		matris_C[0][i]=cols1[i];
